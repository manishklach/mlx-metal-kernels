...
def main():
    ...
    ref_ms = time_fn(lambda: reference_paged_decode_attention(q, K_pages, V_pages, block_table, lengths), iters=args.iters) * 1e3
    backends = ["reference", "metal"] if args.backend == "all" else [args.backend]
    for backend in backends:
        cur_ms = ref_ms if backend == "reference" else time_fn(lambda: paged_decode_attention(q, K_pages, V_pages, block_table, lengths, backend="metal"), iters=args.iters) * 1e3
        speedup = ref_ms / cur_ms if cur_ms > 0 else float("inf")
        print(f"backend={backend} B={args.B} MAX_S={args.MAX_S} PAGE_SIZE={args.PAGE_SIZE} H={args.H} D={args.D} length={lengths} dtype={args.dtype} milliseconds={cur_ms:.3f} speedup_vs_reference={speedup:.2f}x")
