import argparse
import time

import mlx.core as mx

from ops.attention_ops import fast_attention


def _run(backend, Q, K, V, causal, D):
    if backend == "generic":
        return fast_attention(Q, K, V, causal=causal, backend="baseline")
    if backend == "specialized":
        return fast_attention(Q, K, V, causal=causal, backend=f"baseline_d{D}")
    return fast_attention(Q, K, V, causal=causal, backend="reference")


def _time_fn(fn, iters):
    for _ in range(5):
        y = fn()
        mx.eval(y)
    start = time.perf_counter()
    for _ in range(iters):
        y = fn()
        mx.eval(y)
    end = time.perf_counter()
    return (end - start) / iters


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--B", type=int, default=1)
    parser.add_argument("--S", type=int, default=128)
    parser.add_argument("--H", type=int, default=8)
    parser.add_argument("--D", type=int, choices=[64, 128], default=64)
    parser.add_argument("--dtype", choices=["float16", "bfloat16"], default="float16")
    parser.add_argument("--causal", action="store_true")
    parser.add_argument("--backend", choices=["generic", "specialized", "all"], default="all")
    parser.add_argument("--iters", type=int, default=20)
    args = parser.parse_args()

    dtype = mx.float16 if args.dtype == "float16" else mx.bfloat16
    mx.random.seed(113)
    Q = mx.random.normal((args.B, args.S, args.H, args.D)).astype(dtype)
    K = mx.random.normal((args.B, args.S, args.H, args.D)).astype(dtype)
    V = mx.random.normal((args.B, args.S, args.H, args.D)).astype(dtype)
    backends = ["reference", "generic", "specialized"] if args.backend == "all" else [args.backend]
    timings = {}
    for backend in backends:
        timings[backend] = _time_fn(lambda b=backend: _run(b, Q, K, V, args.causal, args.D), args.iters)
        ms = timings[backend] * 1e3
        speed_generic = f"{timings['generic'] / timings[backend]:.3f}" if backend != "generic" and "generic" in timings else "n/a"
        speed_ref = f"{timings['reference'] / timings[backend]:.3f}" if backend != "reference" and "reference" in timings else "n/a"
        print(
            f"backend={backend} B={args.B} S={args.S} H={args.H} D={args.D} dtype={args.dtype} "
            f"causal={args.causal} milliseconds={ms:.3f} speedup_vs_generic={speed_generic} "
            f"speedup_vs_reference={speed_ref}"
        )


if __name__ == "__main__":
    main()
