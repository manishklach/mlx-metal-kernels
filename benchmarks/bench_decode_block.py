...
def run_loop(B, MAX_S, T, H, D, dtype, layout, backend):
    ...
    for pos in range(T):
        ...
        out, K_cache, V_cache = decode_block_from_qkv(
            qkv, K_cache, V_cache, cos, sin, pos, H=H, D=D, backend=backend
        )
...
def main():
    ...
    for backend in backends:
        avg_s = time_fn(
            lambda: run_loop(args.B, args.MAX_S, args.T, args.H, args.D, dtype, args.layout, backend),
            iters=args.iters,
        )
        ...
        print(
            f"backend={backend} B={args.B} MAX_S={args.MAX_S} T={args.T} H={args.H} D={args.D} "
            f"dtype={args.dtype} layout={args.layout} ms_per_step={ms_per_step:.3f} "
            f"tokens_per_second={tps:.3f}{speedup}"
        )
