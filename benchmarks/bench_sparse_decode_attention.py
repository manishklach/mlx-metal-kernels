...
from ops.sparse_attention_ops import SparseAttentionPattern, reference_sparse_gqa_decode_attention, sparse_gqa_decode_attention
...
def main():
    ...
    parser.add_argument("--MAX_S", type=int, default=4096)
    ...
    dense_ref_timing = time_fn(lambda: reference_gqa_decode_attention(q, K_cache, V_cache, lengths=args.length, scale=1.0 / math.sqrt(args.D)), warmup=1, iters=max(1, min(args.iters, 5)))
    ...
    for backend_name in backends:
        ...
        sparse_ref_timing = time_fn(lambda p=pattern: reference_sparse_gqa_decode_attention(q, K_cache, V_cache, args.length, p), warmup=1, iters=max(1, min(args.iters, 5)))
        if backend_name == "reference":
            fn = lambda p=pattern: reference_sparse_gqa_decode_attention(q, K_cache, V_cache, args.length, p)
        else:
            fn = lambda b=backend_name, p=pattern: sparse_gqa_decode_attention(q, K_cache, V_cache, args.length, p, backend=b)
        if args.validate and backend_name != "reference":
            got = sparse_gqa_decode_attention(q, K_cache, V_cache, args.length, pattern, backend=backend_name)
            ref = reference_sparse_gqa_decode_attention(q, K_cache, V_cache, args.length, pattern)
            mx.eval(got, ref)
        ...
        tokens_per_second = (args.B * 1000.0) / timing["mean_ms"]
        print(
            f"B={args.B} MAX_S={args.MAX_S} length={args.length} Hq={args.Hq} Hkv={args.Hkv} D={args.D} "
