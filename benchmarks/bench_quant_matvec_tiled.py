...
def main():
    ...
    parser.add_argument("--backend", choices=["reference", "metal", "metal_parallel", "metal_tiled", "all"], default="all")
    ...
    backends = ["reference", "metal", "metal_parallel", "metal_tiled"] if args.backend == "all" else [args.backend]
    timings = {}
    for backend in backends:
        ...
        speed_parallel = timings["metal_parallel"] / timings[backend] if backend != "metal_parallel" and "metal_parallel" in timings else 1.0
        print(
            ...
            f"speedup_vs_metal_parallel={speed_parallel:.2f}x"
        )
