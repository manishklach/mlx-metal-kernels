...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1, 96, 50, 16, False, mx.float16, False),
        (1, 128, 64, 32, True, mx.bfloat16, False),
        (1, 64, 32, 32, False, mx.float16, True),
    ],
)
def test_q4_matvec_decode_tiled(B, K, N, group_size, use_zero, dtype, use_token_shape):
    mx.random.seed(210)
    ...
    got_tiled = q4_matvec_decode(x, packed, scales, zeros, group_size=group_size, backend="metal_tiled")
    got_parallel = q4_matvec_decode(x, packed, scales, zeros, group_size=group_size, backend="metal_parallel")
    mx.eval(ref, got_tiled, got_parallel)
    atol, rtol = _tolerances(dtype)
    assert mx.allclose(got_tiled, ref, atol=atol, rtol=rtol).item()
    assert mx.allclose(got_tiled, got_parallel, atol=atol, rtol=rtol).item()
...
def test_q8_matvec_decode_tiled(B, K, N, group_size, use_zero, dtype, use_token_shape):
    mx.random.seed(211)
    x = mx.random.normal((B, 1, K)).astype(dtype) if use_token_shape else mx.random.normal((B, K)).astype(dtype)
    ...
    got_tiled = q8_matvec_decode(x, q_w, scales, zeros, group_size=group_size, backend="metal_tiled")
