...
class LlamaLikeKernelAdapter:
    ...
    def validate_supported(self) -> None:
        ...
        if self.config.num_key_value_heads != self.config.num_attention_heads:
            raise NotImplementedError(
                "GQA/MQA is not yet supported by this scaffold. "
                f"Expected num_key_value_heads == num_attention_heads, got {self.config.num_key_value_heads}, "
    ...
    def decode_layer_quantized_from_fused_qkv(
        ...
        *,
        ...
        if self.cache_layout == "contiguous":
            out, updated_K, updated_V = toy_transformer_decode_layer(
                x,
                attn_norm_weight,
                ffn_norm_weight,
                qkv_w,
            ...
            return out, LlamaLayerState(K_cache=updated_K, V_cache=updated_V)
        out, updated_K, updated_V = paged_toy_transformer_decode_layer(
            x,
            attn_norm_weight,
            ffn_norm_weight,
            qkv_w,
