...
const int D = meta[4];
const int D_packed = meta[5];
const int group_size = meta[6];
const int groups_per_head = meta[7];
...
const int kv_stride_b = MAX_S * Hkv * D_packed;
const int kv_stride_s = Hkv * D_packed;
const int kv_stride_h = D_packed;
...
for (int j = 0; j < valid_len; ++j) {
    ...
    for (int d = 0; d < D; ++d) {
        ...
        const int nibble = (d % 2 == 0)
            ? (K_q[kv_base + byte_idx] & 0x0F)
            : ((K_q[kv_base + byte_idx] >> 4) & 0x0F);
    ...
    for (int d = 0; d < D; ++d) {
        const int byte_idx = d / 2;
        const int nibble = (d % 2 == 0)
            ? (V_q[kv_base + byte_idx] & 0x0F)
            : ((V_q[kv_base + byte_idx] >> 4) & 0x0F);
