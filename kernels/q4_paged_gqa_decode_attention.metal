...
const int D = meta[6];
const int D_packed = meta[7];
const int group_size = meta[8];
const int groups_per_head = meta[9];
...
const int page_stride_p = PAGE_SIZE * Hkv * D_packed;
const int page_stride_s = Hkv * D_packed;
const int page_stride_h = D_packed;
...
for (int j = 0; j < valid_len; ++j) {
    ...
    for (int d = 0; d < D; ++d) {
        ...
        const int nibble = (d % 2 == 0)
            ? (K_pages_q[kv_base + byte_idx] & 0x0F)
            : ((K_pages_q[kv_base + byte_idx] >> 4) & 0x0F);
        const float q_val = float(q[q_base + d]);
        const float k_val = (float(nibble) - 8.0f) * K_scales[sc_base + d / group_size];
        dot += q_val * k_val;
    }
    ...
    for (int d = 0; d < D; ++d) {
        const int byte_idx = d / 2;
        const int nibble = (d % 2 == 0)
            ? (V_pages_q[kv_base + byte_idx] & 0x0F)
            : ((V_pages_q[kv_base + byte_idx] >> 4) & 0x0F);
        const float v_val = (float(nibble) - 8.0f) * V_scales[sc_base + d / group_size];
